...
In this study, we leveraged the spatially-aware CERs that better encode multiomics features as intracellular regulatory mediators for better inference of directed signal flows among cells. In the inference process, we employ some of the modules in FlowSig, therefore, the FlowSig package needs to be installed first via https://github.com/axelalmet/flowsig. The spatial signal flow inference module is organized under the directory signal flow network inference module (https://github.com/WHY-17/SpaDDM/tree/main/signal%20flow%20network%20inference%20module), which mainly contains the general-purpose functions required for signal flow network inference as reported in SpaDDM. Within this directory, we additionally provide a tutorial.py script that demonstrates how to use these functions to perform signal flow network inference in practice.
To further facilitate a step-by-step and transparent implementation of signal flow network inference, we also created a separate directory named intercellular flow network inference. This directory contains Jupyter notebooks that walk through the complete inference pipeline in detail, illustrating each intermediate step of the workflow. We take the data of mouse embryonic brain E18.5 as an example to describe the inference process of intercellular signal flow in SpaDDM. The readme in the folder provides a step-by-step introduction to all the calculation processes and the reproduction of the results in the manuscript.
