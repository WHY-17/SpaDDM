...
def add_image(adata, library_id=None, scale=None, quality="hires", spot_diameter_fullres=50, image_hires_path=None, image_lowres_path=None):
    if scale == None:
        max_coor = np.max(adata.obsm["spatial"])
        scale = 2000 / max_coor

    if 'array_row' not in adata.obs:
        adata.obs["imagecol"] = adata.obsm["spatial"][:, 0] * scale
        adata.obs["imagerow"] = adata.obsm["spatial"][:, 1] * scale

    # load image
    image_data_hires = imageio.imread(image_hires_path)

    if image_lowres_path != None:
        image_data_lowres = imageio.imread(image_lowres_path)

    adata.uns["spatial"] = {}
    adata.uns["spatial"][library_id] = {}
    adata.uns["spatial"][library_id]["images"] = {}
    adata.uns["spatial"][library_id]["images"][quality] = image_data_hires

    if image_lowres_path != None:
        adata.uns["spatial"][library_id]["images"]['lowres'] = image_data_lowres
...
def image_feature(adata, pca_components, seeds=2024, verbose=True):
    feat_df = pd.DataFrame()
    with tqdm(total=len(adata),
        ...
        for spot, slice_path in adata.obs['slices_path'].items():
            ...
            spot_slice = transform(spot_slice).flatten()
            feat_df[spot] = spot_slice
            feat_df = feat_df.copy()
            pbar.update(1)
