...
class Train_SpatialDDM:
    def __init__(
        ...
        """
        # feature
        self.features_omics_1 = torch.FloatTensor(self.adata_omics_1.obsm['feat'].copy()).to(self.device)
        self.features_omics_2 = torch.FloatTensor(self.adata_omics_2.obsm['feat'].copy()).to(self.device)
        """
        ...
        if self.datatype == 'SPOTS':
            ...
            self.epochs = 600
            self.weight_factors = [0.5, 0.5, 0.4, 0.1, 0.2]

        elif self.datatype == 'Spatial-epigenome-transcriptome':
            self.epochs = 600
            self.weight_factors = [0.5, 0.5, 0.1, 0.5, 0.5]

        elif self.datatype == 'Spatial-CITE-seq':
            self.epochs = 400
            self.weight_factors = [0.5, 0.5, 0.5, 0.1, 0.2]

        elif self.datatype == 'MISAR':
            self.epochs = 600
            self.weight_factors = [0.5, 0.5, 1.0, 0.1, 0.2]
    ...
    def train(self, save_path):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            self.loss_recon_image = results['loss_rec_image']
            ...
            self.loss_corr_image = F.mse_loss(self.dgl_omics_1.ndata['image'], results['comb_recon_image'])

            print("rec_loss_omics_1:", self.loss_recon_omics_1, "rec_loss_omics_2:", self.loss_recon_omics_2,
                  "rec_loss_image:", self.loss_recon_image,
                  "corr_loss_omics_1:", self.loss_corr_omics_1, "corr_loss_omics_2:", self.loss_corr_omics_2,
                  "corr_loss_image:", self.loss_corr_image
                  )

            loss = (self.weight_factors[0] * self.loss_recon_omics_1 + self.weight_factors[1] * self.loss_recon_omics_2 +
                    self.weight_factors[2] * self.loss_recon_image + self.weight_factors[3] * self.loss_corr_omics_1 +
                    self.weight_factors[4] * self.loss_corr_omics_2
                    )
        ...
        with torch.no_grad():
            self.model.eval()
            results = self.model(self.dgl_omics_1, self.dgl_omics_2)
            torch.save(self.model.state_dict(), save_path+'DDM_model.pth')
